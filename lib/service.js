...
function parseConfig (manager, context) {
  ...
  context.protocal =
    protocal === 'http' ? protocal : protocal === 'https' ? protocal : 'http'
...
function handleInterfaceConfig (context) {
  ...
  try {
    result1 = fs
      .readFileSync(path.resolve(__dirname, '../conf/interface.json'))
      .toString()
  } catch (e) {
    logger.fatal(
      '[init]',
      'Failed to read the configuration interface.json file'
    )
    throw e
  }
  ...
  result1 = parseJSON(
    result1,
    'Parsing configuration interface.json file failed'
  )
