...
function init (context) {
  ...
  try {
    result1 = fs
      .readFileSync(path.resolve(__dirname, '../conf/service.json'))
      .toString()
  } catch (e) {
  ...
  try {
    result2 = fs
      .readFileSync(path.join(context.workpath, 'service.json'))
      .toString()
  } catch (e) {
  ...
  tools.extend(
    context.confs,
    tools.replace(templateServer, applicationServer, true),
    false
  )
